...
public class Main extends JLayeredPane implements KeyListener {
	...
	public void setUpFields(int WIDTH, int HEIGHT, Frame frame) {
		...
		File theDir = new File("C:\\Users\\" + System.getProperty("user.name") + "\\AppData\\Local\\Calendar\\");
		if (!theDir.exists()){
		    theDir.mkdirs();
		}
		path = "C:\\Users\\" + System.getProperty("user.name") + "\\AppData\\Local\\Calendar\\";
	...
	public void setUpComponents() { // setting up JComponents like titles and calling the method to set up day JTextAreas
		...
		scrollPanes = new JScrollPane[7][6];
	...
	public void setUpTextAreas() { // the text areas represent each day
		for(int j = 0; j < 6; j++) {
			for(int i = 0; i < daysOfTheWeek.size(); i++) {
				...
				scrollPanes[i][j].getHorizontalScrollBar().setValue(scrollPanes[i][j].getHorizontalScrollBar().getMinimum());
				scrollPanes[i][j].getVerticalScrollBar().setValue(scrollPanes[i][j].getVerticalScrollBar().getMinimum());
	...
	public void changeMonth(int direction) { // method called when you change months, it saves the current month and loads in the new one
		...
		setFirstDay();
		reformatAllTextBoxes();
		loadTextAreas();
		updateCurrentDateBox();
		new RepositionScrollPanels().start();
		repaintComponents();
	...
	public void reformatAllTextBoxes() {
		for(int j = 0; j < 6; j++) {
			for(int i = 0; i < daysOfTheWeek.size(); i++) {
				boxFormatter(i, j);
	...
	private class RepositionScrollPanels extends Thread {
		public int timer = 0;
		public void run() {
			try {
				while(timer <= 5) {
